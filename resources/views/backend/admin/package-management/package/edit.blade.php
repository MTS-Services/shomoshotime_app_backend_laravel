<x-admin::layout>
    <x-slot name="title">{{ __('Edit Package') }}</x-slot>
    <x-slot name="breadcrumb">{{ __('Edit Package') }}</x-slot>
    <x-slot name="page_slug">package</x-slot>
    <section>
        <div class="glass-card rounded-2xl p-6 mb-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-text-black dark:text-text-white">{{ __('Edit Package') }}</h2>
                <x-admin.primary-link href="{{ route('pam.package.index') }}">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
            <div class="glass-card rounded-2xl p-6 md:col-span-5">
                <form action="{{ route('pam.package.update', encrypt($package->id)) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
                        ...
                        <div class="space-y-3">
                            ...
                            <label class="input flex items-center gap-2">
                                <input type="text" name="total_ad" value="{{ $package->total_ad }}"
