<x-admin::layout>
    <x-slot name="title">{{ __('Create Area') }}</x-slot>
    <x-slot name="breadcrumb">{{ __('Create Area') }}</x-slot>
    <x-slot name="page_slug">area</x-slot>
    <section>
        <div class="glass-card rounded-2xl p-6 mb-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-text-black dark:text-text-white">{{ __('Create Area') }}</h2>
